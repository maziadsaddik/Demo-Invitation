    ...
    public class AcceptTesting : IClassFixture<WebApplicationFactory<Program>>
    {
       ...
       string memberId,
       ...
       )
        {
            ...
            AssertEquality.OfAcceptedEvent(events[1], invitationRequest.InvitationInfo, response, expectedSequence: 2);
        }
        [Theory]
        ...
        public async Task Accept_SendInvalidRequest_ThrowsInvalidArgumentRpcException(
           bool validaccountId,
           bool validuserId,
           bool validmemberId,
       ...
       )
        {
            ...
            var exception = await Assert.ThrowsAsync<RpcException>(async () => await client.AcceptAsync(request.InvitationInfo));
